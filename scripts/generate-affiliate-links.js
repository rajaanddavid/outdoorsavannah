...
document.addEventListener("DOMContentLoaded", async function() {
    ...
    function showFallbackButton(deeplink_android) {
        ...
        overlay.id = 'safari16-fallback-overlay';
        ...
        container.style.cssText = "background: rgba(0, 0, 0, 0.95); padding: 30px; border-radius: 12px; text-align: center; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; z-index: 999999; box-shadow: 0 8px 32px rgba(0, 0, 0, 0.3); max-width: 90%;";

        // Prevent clicks on modal from closing the overlay
        container.addEventListener('click', (e) => {
            e.stopPropagation();
        });

        const message = document.createElement('p');
        message.textContent = 'App not installed';
        message.style.cssText = "color: white; font-size: 18px; margin: 0 0 20px 0;";

        const button = document.createElement('a');
        button.href = deeplink_android;
        button.textContent = 'Continue to destination';
        button.style.cssText = "display: inline-block; padding: 14px 28px; background: #0073e6; color: white; text-decoration: none; border-radius: 8px; font-weight: 600; font-size: 16px;";
    ...
    if (isAndroid()) {
        if (isAppBrowser()) {
            ...
            showFallbackButton(deeplink_android);
